...
class FederatedClient:
    ...
    def setup_logging(self):
        ...
        logger.setLevel(logging.DEBUG)  # Set root logger to DEBUG to capture all logs
        ...
        file_handler = logging.FileHandler(f'federated_client_{self.client_id}.log')
        ...
        logging.getLogger('src.lora.federated_lora').setLevel(logging.DEBUG)
    ...
    async def train_task_with_kd(self, task: str, task_data: Dict) -> Dict[str, float]:
        ...
        for batch_idx, batch in enumerate(train_dataloader):
            try:
                ...
                # Log parameter statistics before forward pass
                if num_batches % 10 == 0:  # Log every 10 batches to reduce noise
                    logger.info("\nPARAMETER STATISTICS BEFORE UPDATE:")
                    total_params = 0
                    trainable_params = 0
                    for name, param in self.student_model.named_parameters():
                        if param.requires_grad:
                            ...
                            if 'lora' in name.lower():
                                logger.info(f"LoRA param {name}: mean={param.data.mean().item():.6f}, std={param.data.std().item():.6f}")
                        total_params += param.numel()
                    logger.info(f"Total parameters: {total_params:,}, Trainable: {trainable_params:,} ({trainable_params/total_params*100:.2f}%)")
                ...
                if num_batches % 10 == 0:  # Log every 10 batches to reduce noise
                    ...
                    for name, param in self.student_model.named_parameters():
                        if param.grad is not None and param.requires_grad:
                            has_grad = True
                            if 'lora' in name.lower():
                                logger.info(f"LoRA grad {name}: mean={param.grad.mean().item():.6f}, std={param.grad.std().item():.6f}")
                    if not has_grad:
                        logger.warning("No gradients found for any parameters! Check if requires_grad is set correctly.")
                ...
                if num_batches % 10 == 0:  # Log every 10 batches to reduce noise
                    ...
                    for name, param in self.student_model.named_parameters():
                        if param.requires_grad and 'lora' in name.lower():
                            logger.info(f"Updated LoRA param {name}: mean={param.data.mean().item():.6f}, std={param.data.std().item():.6f}")
                    logger.info("-" * 80)

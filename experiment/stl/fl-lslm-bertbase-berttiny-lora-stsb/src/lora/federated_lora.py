...
class LoRAFederatedModel(nn.Module):
    ...
    def __init__(self, base_model_name: str, tasks: List[str], lora_rank: int = 32, lora_alpha: float = 64.0, unfreeze_layers: int = 2):
        ...
        # Log model loading and configuration
        logger.info("="*60)
        logger.info("INITIALIZING LoRA FEDERATED MODEL")
        ...
        self.base_model = AutoModelForSequenceClassification.from_pretrained(
            base_model_name,
            num_labels=1,  # For KD compatibility
            return_dict=True,
        ...
        logger.info(f"Model architecture: {type(self.base_model).__name__}")
        if hasattr(self.base_model, 'bert') and hasattr(self.base_model.bert, 'encoder') and hasattr(self.base_model.bert.encoder, 'layer'):
            logger.info(f"Number of transformer layers: {len(self.base_model.bert.encoder.layer)}")
        
        # Log LoRA configuration
        logger.info("\nCONFIGURING LoRA ADAPTERS")
        logger.info(f"LoRA rank (r): {lora_rank}")
        logger.info(f"LoRA alpha: {lora_alpha}")
        logger.info("Base model parameters will be frozen, LoRA parameters will be trainable")
        
        # Log trainable parameters before freezing
        trainable_before = sum(p.numel() for p in self.base_model.parameters() if p.requires_grad)
        logger.info(f"Trainable parameters before freezing: {trainable_before:,}")
        ...
        for name, param in self.base_model.named_parameters():
            ...
            frozen_count += 1
            if frozen_count <= 5:  # Only log first few parameters to avoid spam
        ...
        logger.info("\nMODEL PARAMETER STRUCTURE (first 10 parameters):")
        for i, (name, param) in enumerate(self.base_model.named_parameters()):
            if i >= 10:  # Only show first 10 parameters
                logger.info(f"... and {sum(1 for _ in self.base_model.parameters()) - 10} more parameters")
                break

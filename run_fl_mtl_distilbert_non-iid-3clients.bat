...
REM DistilBERT with 3 clients per task (9 total clients)
...
set CONDA_ENV=py312
set PROJECT_ROOT=C:\Users\hunglq\docs\FedAvgLS\experiment_new_solution\models\distil-bert\fl-mtl-slms-disti-bert-non-iid-sst2-qqp-sst2-3client-each
...
echo Starting Federated Server (DistilBERT MTL Non-IID)...
start "FL-MTL Server" cmd /k ^
"%CONDA_ROOT%\condabin\conda.bat activate %CONDA_ENV% ^&^& cd /d %WORK_DIR% ^&^& set CUDA_VISIBLE_DEVICES=0 ^&^& python federated_main.py --mode server --port 8772"

echo Server started on port 8772
echo Waiting 5 seconds for server to initialize...
timeout /t 5 >nul

REM ===== SST2 Clients =====
echo Starting SST2 Client 1 (DistilBERT Non-IID)...
...
echo Starting SST2 Client 2 (DistilBERT Non-IID)...
...
echo Starting SST2 Client 3 (DistilBERT Non-IID)...
...
REM ===== QQP Clients =====
echo Starting QQP Client 1 (DistilBERT Non-IID)...
...
echo Starting QQP Client 2 (DistilBERT Non-IID)...
...
echo Starting QQP Client 3 (DistilBERT Non-IID)...
...
REM ===== STSB Clients =====
echo Starting STSB Client 1 (DistilBERT Non-IID)...
...
echo Starting STSB Client 2 (DistilBERT Non-IID)...
...
echo Starting STSB Client 3 (DistilBERT Non-IID)...

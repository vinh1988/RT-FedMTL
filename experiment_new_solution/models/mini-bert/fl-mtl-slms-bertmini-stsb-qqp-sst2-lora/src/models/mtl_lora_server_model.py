...
class MTLLoRAServerModel(nn.Module):
    ...
    def __init__(self, base_model_name: str, tasks: List[str], 
        ...
        logger.info(f"BERT layers: {self.num_layers} total, {layers_to_freeze} frozen, {self.num_layers - layers_to_freeze} unfrozen")
        logger.info(f"Frozen BERT params: {frozen_params:,}, Unfrozen BERT params: {unfrozen_params:,}")
        ...
        for i, layer in enumerate(self.bert.encoder.layer):
            if i >= self.layers_to_freeze:
                for name, param in layer.named_parameters():
                    params[f"bert.encoder.layer.{i}.{name}"] = param.data.clone()
        return params
    ...
    def set_unfrozen_layer_parameters(self, parameters: Dict[str, torch.Tensor]):
        ...
        with torch.no_grad():
            for i, layer in enumerate(self.bert.encoder.layer):
                if i >= self.layers_to_freeze:
                    for name, param in layer.named_parameters():
                        key = f"bert.encoder.layer.{i}.{name}"

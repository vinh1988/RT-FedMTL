...
from src.models.mtl_lora_server_model import MTLLoRAServerModel
from src.aggregation.mtl_lora_aggregator import MTLLoRAAggregator
...
class FederatedServer:
    ...
    def __init__(self, config: FederatedConfig):
        ...
        # Initialize MTL LoRA Server Model with Partial Layer Unfreezing
        # (frozen bottom layers + unfrozen top layers + shared LoRA + task-specific heads)
        self.mtl_model = MTLLoRAServerModel(
            ...
            layers_to_freeze=getattr(config, 'layers_to_freeze', 2)  # Partial unfreezing
        )
        logger.info("=" * 60)
        logger.info("MTL LoRA Server Model Summary (Partial Unfreezing):")
        summary = self.mtl_model.get_model_summary()
        logger.info(f"  Base Model: {summary['base_model']}")
        logger.info(f"  Tasks: {summary['tasks']}")
        logger.info(f"  BERT Layers: {summary['num_layers']} total ({summary['layers_frozen']} frozen, {summary['layers_unfrozen']} unfrozen)")
        logger.info(f"  Frozen BERT: {summary['frozen_bert_parameters']:,} params")
        logger.info(f"  Unfrozen BERT: {summary.get('unfrozen_bert_parameters', 0):,} params (trainable)")
        logger.info(f"  Shared LoRA: {summary['shared_lora_parameters']:,} params (trainable)")
        for task, size in summary['task_head_parameters'].items():
            logger.info(f"  Task '{task}' Head: {size:,} parameters")
        logger.info(f"  Total Trainable: {summary['total_trainable_parameters']:,}")
        logger.info(f"  Trainable Ratio: {summary['trainable_ratio']}")
        logger.info("=" * 60)

        # Initialize MTL LoRA-aware aggregator
        self.aggregator = MTLLoRAAggregator()
        ...
        for round_num in range(1, self.config.num_rounds + 1):
            ...
            try:
                ...
                if round_num in self.client_updates and self.client_updates[round_num]:
                    ...
                    # Perform MTL LoRA-aware aggregation
                    aggregated = self.aggregator.aggregate_mtl_lora_updates(client_updates)
                    
                    # Update MTL LoRA server model
                    if aggregated['shared_lora']:
                        self.mtl_model.set_shared_lora_parameters(aggregated['shared_lora'])
                        logger.info(f"✓ Updated shared LoRA adapters with {len(aggregated['shared_lora'])} parameters")
    ...
    def finalize_training(self):
        ...
        try:
            ...
            with open(summary_file, 'w') as f:
                ...
                # Add MTL LoRA model summary with partial unfreezing
                mtl_summary = self.mtl_model.get_model_summary()
                f.write(f"\nMTL LoRA Model Architecture (Partial Unfreezing):\n")
                f.write(f"  Base Model: {mtl_summary['base_model']}\n")
                f.write(f"  Tasks: {', '.join(mtl_summary['tasks'])}\n")
                f.write(f"  BERT Layers: {mtl_summary['num_layers']} total ({mtl_summary['layers_frozen']} frozen, {mtl_summary['layers_unfrozen']} unfrozen)\n")
                f.write(f"  Frozen BERT Parameters: {mtl_summary['frozen_bert_parameters']:,}\n")
                f.write(f"  Unfrozen BERT Parameters: {mtl_summary.get('unfrozen_bert_parameters', 0):,}\n")
                f.write(f"  Shared LoRA Parameters: {mtl_summary['shared_lora_parameters']:,}\n")
                for task, size in mtl_summary['task_head_parameters'].items():
                    f.write(f"  Task '{task}' Head: {size:,} parameters\n")
                f.write(f"  Total Trainable Parameters: {mtl_summary['total_trainable_parameters']:,}\n")
                f.write(f"  Trainable Ratio: {mtl_summary['trainable_ratio']}\n")

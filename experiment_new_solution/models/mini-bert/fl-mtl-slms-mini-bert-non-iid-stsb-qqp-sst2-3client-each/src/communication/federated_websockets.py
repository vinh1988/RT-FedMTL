...
class MessageProtocol:
    ...
    def serialize_tensors(data: Dict) -> Dict:
        """Convert tensors to JSON-serializable format with compression"""
        if isinstance(data, dict):
            ...
            # Use numpy array for more efficient serialization
            return {
                "__tensor__": True,
                "data": data.detach().cpu().numpy().tobytes().hex(),  # More compact than tolist()
    ...
    def deserialize_tensors(data: Dict, device: str = "cpu") -> Dict:
        ...
        if isinstance(data, dict):
            if "__tensor__" in data:
                ...
                # Convert hex string back to numpy array, then to tensor
                import numpy as np
                hex_data = data["data"]
                numpy_array = np.frombuffer(bytes.fromhex(hex_data), dtype=np.float32)

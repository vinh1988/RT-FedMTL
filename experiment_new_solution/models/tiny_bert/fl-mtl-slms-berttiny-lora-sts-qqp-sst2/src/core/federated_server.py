...
from src.models.peft_lora_model import PEFTLoRAServerModel
from src.aggregation.mtl_aggregator import MTLAggregator
from src.aggregation.peft_lora_aggregator import PEFTLoRAAggregator
...
from src.synchronization.peft_lora_synchronization import PEFTLoRASynchronizationManager
...
class FederatedServer:
    ...
    def __init__(self, config: FederatedConfig):
        ...
        # Check if PEFT LoRA is enabled
        self.use_peft_lora = getattr(config, 'use_peft_lora', False)
        
        if self.use_peft_lora:
            # Initialize PEFT LoRA Server Model
            logger.info("=" * 60)
            logger.info("INITIALIZING PEFT LoRA MTL SERVER")
            logger.info("=" * 60)
            ...
            self.peft_lora_model = PEFTLoRAServerModel(
                ...
                unfreeze_layers=getattr(config, 'lora_unfreeze_layers', 2)
            )
            
            summary = self.peft_lora_model.get_model_summary()
            logger.info("PEFT LoRA Server Model Summary:")
            logger.info(f"  Base Model: {summary['base_model']}")
            logger.info(f"  Tasks: {summary['tasks']}")
            logger.info(f"  LoRA Rank: {summary['lora_rank']}")
            logger.info(f"  LoRA Alpha: {summary['lora_alpha']}")
            logger.info(f"  Target Modules: {summary['target_modules']}")
            ...
            # Initialize PEFT LoRA aggregator
            self.aggregator = PEFTLoRAAggregator()
            self.synchronization_manager = PEFTLoRASynchronizationManager(self)
            
        else:
            # Initialize standard MTL Server Model (shared BERT + task-specific heads)
            logger.info("=" * 60)
    ...
    async def run_federated_training(self):
        """Main federated training loop with MTL or PEFT LoRA"""
        if self.use_peft_lora:
            logger.info("Starting Federated Learning with PEFT LoRA Multi-Task Learning")
            logger.info("Architecture: Shared BERT + Task-Specific LoRA Adapters")
        else:
        ...
        # Training rounds with MTL-aware or PEFT LoRA aggregation
        for round_num in range(1, self.config.num_rounds + 1):
            logger.info(f"=== Round {round_num}/{self.config.num_rounds} ===")
            round_start = time.time()

            try:
                # Step 1: Broadcast model/adapters to clients
                if self.config.enable_synchronization:
                    if self.use_peft_lora:
                        await self.broadcast_peft_lora_adapters()
                    else:
                ...
                # Step 4: Aggregation (different logic for PEFT LoRA vs standard MTL)
                if round_num in self.client_updates and self.client_updates[round_num]:
                    client_updates = self.client_updates[round_num]
                    
                    if self.use_peft_lora:
                        # PEFT LoRA aggregation: aggregate LoRA adapter parameters
                        aggregated_lora = self.aggregator.aggregate_lora_updates(client_updates)
                        
                        if aggregated_lora:
                            self.peft_lora_model.update_from_aggregation(aggregated_lora)
                            logger.info(f"Updated PEFT LoRA adapters with {len(aggregated_lora)} parameters")
                            self.synchronization_manager.increment_model_version()
                    else:
                        # Standard MTL aggregation
    ...
    async def broadcast_peft_lora_adapters(self):
        """
        Broadcast PEFT LoRA adapters to clients
        Much more efficient than full model broadcast
        """
        if not self.use_peft_lora:
            logger.warning("PEFT LoRA not enabled, using standard MTL broadcast")
            return await self.broadcast_mtl_model_slices()
        
        logger.info("Broadcasting PEFT LoRA adapters to clients...")
        ...
        for client_id, client_info in self.connected_clients.items():
            ...
            # Get LoRA adapters for this task
            lora_state = self.synchronization_manager.get_lora_state(task=client_task)
            ...
            if client_websocket:
                await client_websocket.send(json.dumps(sync_message))
                logger.debug(f"Sent LoRA adapters for task '{client_task}' to client {client_id}")
        
        logger.info(f"Broadcasted LoRA adapters to {len(self.connected_clients)} clients")
    ...
    def finalize_training(self):
        ...
        try:
            ...
            with open(summary_file, 'w') as f:
                ...
                # Check which model type is being used
                if self.use_peft_lora:
                    f.write(f"Configuration: PEFT LoRA Federated MTL\n")
                    f.write(f"Architecture: LoRA Adapters + Task-Specific Heads\n")
                    f.write(f"Total Rounds: {self.config.num_rounds}\n")
                    f.write(f"Total Clients: {len(self.connected_clients)}\n")
                    
                    # Add PEFT LoRA model summary
                    model_summary = self.peft_lora_model.get_model_summary()
                    f.write(f"\nPEFT LoRA Model Architecture:\n")
                    f.write(f"  Base Model: {model_summary['base_model']}\n")
                    f.write(f"  Tasks: {', '.join(model_summary['tasks'])}\n")
                    f.write(f"  LoRA Rank: {model_summary['lora_rank']}\n")
                    f.write(f"  LoRA Alpha: {model_summary['lora_alpha']}\n")
                    f.write(f"  Target Modules: {', '.join(model_summary['target_modules'])}\n")
                    f.write(f"  Total Parameters: {model_summary['total_parameters']:,}\n")
                    f.write(f"  Trainable Parameters: {model_summary['trainable_parameters']:,}\n")
                    f.write(f"  Trainable Percentage: {model_summary['trainable_percentage']:.2f}%\n")
                else:
                    ...
                    # Add MTL model summary (only for non-LoRA mode)

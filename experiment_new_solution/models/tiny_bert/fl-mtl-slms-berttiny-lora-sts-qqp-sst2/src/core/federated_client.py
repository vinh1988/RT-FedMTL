...
from src.models.peft_lora_model import PEFTLoRAMTLModel
...
from src.synchronization.peft_lora_synchronization import ClientPEFTLoRASynchronizer
...
class FederatedClient:
    ...
    def __init__(self, client_id: str, tasks: List[str], config: FederatedConfig):
        ...
        if self.use_peft_lora:
            logger.info("=" * 60)
            logger.info(f"CLIENT {client_id}: Initializing PEFT LoRA Model")
            logger.info("=" * 60)
        ...
        logger.info(f"Optimizer initialized with {len(trainable_params)} trainable parameter tensors")
        ...
        if self.use_peft_lora:
            self.model_synchronizer = ClientPEFTLoRASynchronizer(
    ...
    def setup_message_handlers(self):
        ...
        self.websocket_client.register_message_handler(
            "lora_sync", self.handle_global_model_sync  # LoRA sync uses same handler
        )
        self.websocket_client.register_message_handler(
    ...
    async def handle_training_request(self, data: Dict):
        ...
        try:
            ...
            if self.use_peft_lora:
                ...
                logger.info(f"Extracted {len(model_updates)} full model parameters")

            # Send update to server (include task label for MTL/LoRA aggregation)
            if self.use_peft_lora:
                # For PEFT LoRA, send LoRA parameters with task label
                update_message = {
                    "type": "client_update",
                    "client_id": self.client_id,
                    "round": round_num,
                    "lora_parameters": model_updates,

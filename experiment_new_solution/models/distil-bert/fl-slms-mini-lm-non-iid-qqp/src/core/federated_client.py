...
from src.datasets.federated_datasets import DatasetFactory, DatasetConfig, extract_client_number
...
class FederatedClient:
    ...
    def initialize_dataset_handlers(self) -> Dict[str, Any]:
        ...
        for task in self.tasks:
            # Use validation_samples from config if available, otherwise default
            val_samples = getattr(self.config, 'validation_samples', min(100, self.config.samples_per_client // 5))
            dataset_config = DatasetConfig(
                task_name=task,
                train_samples=self.config.samples_per_client,
    ...
    async def validate_global_model(self) -> Dict[str, float]:
        ...
        for task in self.tasks:
            ...
            # Get validation data with Non-IID support
            dataset = self.dataset_handlers[task]
            client_num = extract_client_number(self.client_id)
            task_data = dataset.prepare_data(
                num_clients=self.config.max_clients,
                client_id=client_num,
                use_non_iid=self.config.data_distribution == "non_iid",
                non_iid_alpha=getattr(self.config, 'non_iid_alpha', 0.5)
            )
            val_data = {
    ...
    async def perform_local_training(self) -> Dict[str, float]:
        ...
        for task in self.tasks:
            if task in self.dataset_handlers:
                # Get data for this task with Non-IID support
                dataset = self.dataset_handlers[task]
                client_num = extract_client_number(self.client_id)
                task_data = dataset.prepare_data(
                    num_clients=self.config.max_clients,

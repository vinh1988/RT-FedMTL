...
All columns prefixed with `global_model_val_*` represent **global aggregated MTL model** evaluated on **each client's local validation dataset**.
...
3. **Monitor Results:**
   ```bash
   # Check global model validation metrics
   head client_results.csv
   
   # Look for global_model_val_* columns
   grep "global_model_val" client_results.csv
   ```

### **Expected Results:**
- **Round 1**: Empty global_model_val_* (no global model yet)
- **Round 2+**: Populated global_model_val_* columns
- **Improvement**: global_model_val_* > val_* indicates FL benefit

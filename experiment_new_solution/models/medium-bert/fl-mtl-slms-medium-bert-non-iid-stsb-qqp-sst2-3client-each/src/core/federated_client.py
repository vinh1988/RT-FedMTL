...
class FederatedClient:
    ...
    async def validate_global_model(self) -> Dict[str, float]:
        ...
        for task in self.tasks:
            ...
            client_num = extract_client_number(self.client_id)  # Extract numeric client ID
            task_data = dataset.prepare_data(
                num_clients=self.config.max_clients,  # Total clients for this task
                client_id=client_num,         # This client's numeric ID
                use_non_iid=self.config.data_distribution == "non_iid",  # Use Non-IID if configured
                non_iid_alpha=getattr(self.config, 'non_iid_alpha', 0.5)  # Get alpha parameter
            )
            val_data = {

...
class CentralizedTrainer:
    ...
    def _save_metrics_to_csv(self, epoch: int, val_results: Dict, training_time: float, train_loss: float):
        ...
        # Calculate training accuracy for regression (tolerance-based)
        train_accuracy = max(0.0, min(1.0, val_results['metrics'].get('pearson', 0.0)))
        ...
        row = [
            ...
            0.0,  # sst2_val_accuracy (empty for STSB-only)
            0.0,  # sst2_val_f1 (empty for STSB-only)
            0.0,  # qqp_val_accuracy (empty for STSB-only)
            0.0,  # qqp_val_f1 (empty for STSB-only)
            val_results['metrics']['pearson'],  # stsb_val_pearson
            val_results['metrics']['spearman'],  # stsb_val_spearman
    ...
    def train(self):
        ...
        for epoch in range(epochs):
            ...
            # Save checkpoint (disabled per user request)
            # if (epoch + 1) % self.config['output']['checkpoint_interval'] == 0:
            #     self._save_checkpoint(epoch + 1, val_results)
            
            # Save best model (disabled per user request)
            # if val_results['loss'] < self.best_val_loss:
            #     self.best_val_loss = val_results['loss']
            #     self._save_best_model(val_results)
    ...
    def _validate(self) -> Dict:
        ...
        with torch.no_grad():
            for batch in tqdm(self.val_loader, desc="Validation"):
                ...
                # Collect predictions - STSB is regression, use logits directly
                predictions = outputs['logits']
                if predictions.dim() > 1 and predictions.size(-1) == 1:
                    predictions = predictions.squeeze(-1)  # Remove last dimension for regression
    ...
    def _save_checkpoint(self, epoch: int, val_results: Dict):
        ...
        checkpoint_path = self.results_dir / f"checkpoint_epoch_{epoch}.pt"
        torch.save(checkpoint, checkpoint_path)
        self.logger.info(f"Checkpoint saved: {checkpoint_path}")
    
    def _save_best_model(self, val_results: Dict):
        """Save best model"""

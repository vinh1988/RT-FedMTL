...
class FederatedConfig:
    ...
    # Model settings
    server_model: str = "prajjwal1/bert-medium"
    client_model: str = "prajjwal1/bert-medium"
    ...
    # Output settings
    results_dir: str = "fl_non_iid_qqp_result"
    log_level: str = "INFO"
    ...
    def from_yaml_file(cls, config_path: str) -> 'FederatedConfig':
        ...
        print(f"[DEBUG] YAML loaded: {list(config_dict.keys())}")
        if 'training' in config_dict:
            print(f"[DEBUG] YAML training section: {config_dict['training']}")

        # Flatten nested dictionary to match dataclass field names
        flattened_config = cls._flatten_config_dict(config_dict)
        print(f"[DEBUG] Flattened config keys: {list(flattened_config.keys())}")
        if 'validation_samples' in flattened_config:
            print(f"[DEBUG] Found validation_samples in flattened: {flattened_config['validation_samples']}")
        else:
            print(f"[DEBUG] validation_samples NOT found in flattened config")
    ...
    def _flatten_config_dict(cls, config_dict: Dict) -> Dict:
        ...
        key_mapping = {
            ...
            # Data settings
            ('training', 'samples_per_client'): 'samples_per_client',
            ('training', 'validation_samples'): 'validation_samples',
            ('training', 'max_samples_per_client'): 'max_samples_per_client',
            ('training', 'data_distribution'): 'data_distribution',
            ('training', 'non_iid_alpha'): 'non_iid_alpha',
            ('training', 'oversample_minority'): 'oversample_minority',
            ('training', 'normalize_weights'): 'normalize_weights',
    ...
    def get_dataset_configs(self, tasks: List[str]) -> Dict[str, Any]:
        ...
        # Debug: Check if validation_samples is loaded
        print(f"[DEBUG] Config validation_samples: {getattr(self, 'validation_samples', 'NOT_FOUND')}")
        print(f"[DEBUG] Config attributes: {[attr for attr in dir(self) if 'validation' in attr.lower()]}")

        configs = {}
        for task in tasks:
            # Use validation_samples if available, otherwise stop
            val_samples = getattr(self, 'validation_samples', None)
            if val_samples is None:
                print(f"[ERROR] validation_samples not found for task {task}! Stopping execution.")
                raise ValueError(f"validation_samples must be set in config for task {task}")
            print(f"[DEBUG] Task {task}: Using validation_samples={val_samples}")

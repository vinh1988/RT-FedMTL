#!/usr/bin/env python3
"""
Centralized Training Script for Single Task SST2
...
from src.utils.metrics import compute_sst2_metrics
...
def main():
    ...
    # Setup logging
    logger = setup_logger(
        name="centralized_sst2",
        level=config["output"]["log_level"],
        log_dir=config["output"]["results_dir"]
    )
    
    logger.info("=" * 60)
    logger.info("CENTRALIZED TRAINING - SST2 SINGLE TASK")
    logger.info("=" * 60)
    ...
    model = BERTMediumModel(
        ...
        num_labels=2,  # SST2 is binary classification
    ...
    trainer = CentralizedTrainer(
        ...
        device=device,
        config=config,
        metrics_fn=compute_sst2_metrics,
        logger=logger
    )
    ...
    logger.info("Centralized SST2 training completed successfully!")

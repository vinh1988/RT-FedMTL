#!/usr/bin/env python3
"""
Centralized Training Script for Single Task QQP
...
from src.models.bert_medium_model import BERTMediumModel
from src.datasets.qqp_dataset import QQPDataset
from src.training.trainer import CentralizedTrainer
from src.utils.logger import setup_logger
from src.utils.metrics import compute_qqp_metrics
...
def main():
    ...
    # Setup logging
    logger = setup_logger(
        name="centralized_qqp",
        level=config["output"]["log_level"],
        log_dir=config["output"]["results_dir"]
    )
    
    logger.info("=" * 60)
    logger.info("CENTRALIZED TRAINING - QQP SINGLE TASK")
    logger.info("=" * 60)
    ...
    model = BERTMediumModel(
        model_name=config['model']['model_name'],
        num_labels=2,  # QQP is binary classification
    ...
    trainer = CentralizedTrainer(
        ...
        device=device,
        config=config,
        metrics_fn=compute_qqp_metrics,
        logger=logger
    )
    ...
    logger.info("Centralized QQP training completed successfully!")

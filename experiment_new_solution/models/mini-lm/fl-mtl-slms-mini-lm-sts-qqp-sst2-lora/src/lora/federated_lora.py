...
class LoRAFederatedModel(nn.Module):
    ...
    def __init__(self, base_model_name: str, tasks: List[str], lora_rank: int = 32, lora_alpha: float = 64.0, unfreeze_layers: int = 2):
        ...
        if unfreeze_layers > 0:
            ...
            # Try to unfreeze top transformer layers (works for BERT, RoBERTa, MiniLM, DistilBERT, etc.)
            if hasattr(self.base_model, 'bert'):
                # For BERT-based models (including MiniLM)
                ...
                print(f"[SUCCESS] Unfroze top {layers_to_unfreeze} BERT/MiniLM layers + pooler + classifier")
                ...
                for layer in transformer.layer[-layers_to_unfreeze:]:
                    for param in layer.parameters():
                        param.requires_grad = True
                        trainable_params += param.numel()
                    total_params += sum(p.numel() for p in layer.parameters())
                
                # DistilBERT doesn't have a pooler, but has classifier
                if hasattr(self.base_model, 'classifier'):
                    for param in self.base_model.classifier.parameters():
                        param.requires_grad = True
                        trainable_params += param.numel()
                ...
                print(f"[SUCCESS] Unfroze top {layers_to_unfreeze} DistilBERT layers + classifier")
                print(f"[STATS] Trainable parameters in unfrozen layers: {trainable_params:,}")

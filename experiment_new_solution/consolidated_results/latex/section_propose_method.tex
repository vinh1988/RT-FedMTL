...
\subsection{Small Language Model Backbone}
Special SLMs with the proposed framework are adapted to trade off computational efficiency and language fluency. The current solution is compatible with DistilBERT \cite{sanh2019distilbert}, BERT-Medium \cite{turc2019compact}, MiniLM \cite{wang2020minilm} architectures (based on BERT \cite{devlin2018bert}) as well. These are common feature extractors (encoder) that are jointly tuned across the federated network. The communication overhead of our SLMs greatly reduces compared to typical transformers of a standard size.

...
The proposed RT-FMTL-SLM addresses the dual challenges of privacy-preserving NLU and resource-constrained edge computing. Our approach architecture integrates three core pillars: (1) a hierarchical federated architecture optimized for asynchronous real-time synchronization via WebSockets, (2) a multi-task learning strategy that leverages shared linguistic representations while maintaining task-specific specialization, and (3) the deployment of parameter-efficient SLMs to minimize the computational and communication footprint on edge devices.
